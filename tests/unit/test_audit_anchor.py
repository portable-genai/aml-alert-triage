...
from aml_alert_triage.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_CASE_IDS = ("FCC-1001", "FCC-1002", "FCC-1003")

_ANCHOR_ENV = "AMLTRIAGE_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> Container:
    ...
    for alert_id in _CASE_IDS:
        service.triage(ALERTS_BY_ID[alert_id], actor=sample_cases.ACTOR)
    return container


def _audit_of(container: Container) -> LocalAuditAdapter:
...
def test_the_configured_anchor_is_actually_written(tmp_path: Path) -> None:
    ...
    audit = _audit_of(_record_three(settings))
...
def test_a_truncated_tail_is_detected_because_of_the_anchor(tmp_path: Path) -> None:
    audit = _audit_of(_record_three(_durable_settings(tmp_path, anchored=True)))
    assert audit.verify().ok
...
def test_without_the_anchor_the_same_truncation_is_invisible(tmp_path: Path) -> None:
    ...
    audit = _audit_of(_record_three(_durable_settings(tmp_path, anchored=False)))
    _truncate_tail(audit)
    report = audit.verify()
    assert report.ok, (
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    audit = _audit_of(container)
    anchored_before = Path(settings.audit_anchor_path).read_text(encoding="utf-8")

    _truncate_tail(audit)

    with pytest.raises(AuditChainError):
        container.triage_service().triage(ALERTS_BY_ID["FCC-1001"], actor=sample_cases.ACTOR)
