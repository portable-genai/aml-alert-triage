...
from aml_alert_triage.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from aml_alert_triage.config import Settings
from aml_alert_triage.ports.identity import (
    VERIFIED,
...
_API_MODULE = "aml_alert_triage.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        "adapters:",
        "  alert_feed:",
        *[f"    {p}: {_PKG}.adapters.local.alert_feed:LocalAlertFeed" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.alert_feed:OnPremAlertFeed",
        "  warehouse:",
        *[f"    {p}: {_PKG}.adapters.local.warehouse:LocalWarehouse" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.warehouse:OnPremWarehouse",
        "  retrieval:",
        *[f"    {p}: {_PKG}.adapters.local.retrieval:LocalRetrieval" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.retrieval:OnPremRetrieval",
        "  narration:",
        *[f"    {p}: {_PKG}.adapters.local.narration:LocalNarration" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.narration:OnPremNarration",
        "  audit:",
...
def test_a_verified_assertion_is_served_so_the_refusals_above_mean_something(
    ...
    """
    # The `hd` claim IS the tenant this adapter derives, and the alert feed matches it against
    # each row's data tag, so the positive control has to assert a principal who actually owns
    # the fixture rows. A control that 404s on object-level authorization would still "pass" the
    # identity assertions while proving nothing about them.
