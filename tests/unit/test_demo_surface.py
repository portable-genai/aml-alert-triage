...
def test_the_demo_data_is_obviously_fictional() -> None:
    ...
    alerts = [
        ALERTS_BY_ID[demo.ROUTINE_ALERT_ID],
        ALERTS_BY_ID[demo.ESCALATING_ALERT_ID],
        ALERTS_BY_ID[demo.PII_ALERT_ID],
    ]
    for alert in alerts:
        assert "FICTIONAL" in alert.subject, alert.subject + " does not announce itself as fake"
    blob = " ".join(alert.narrative for alert in alerts) + demo.ACTOR
