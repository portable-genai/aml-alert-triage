...
def _persisted_rows() -> list[dict[str, Any]]:
    """Run the REAL pipeline over the fixture alert that carries a planted id; return its rows."""
    container = build_container(Settings(profile="local", audit_path=":memory:"))
    alert = container.alert_feed.fetch_alert("FCC-1001", tenant=FIXTURE_TENANT)
    container.triage_service().triage(alert, actor=sample_cases.ACTOR)
...
def _unredacted(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in mutated:
        ...
        if citations:
            citations[0]["snippet"] = f"Relationship manager NRIC {sample_cases.PLANTED_NRIC} noted"
    return mutated
