...
def test_each_lockfile_keeps_the_header_uv_would_have_destroyed(name: str) -> None:
    """`uv export`, like the `uv pip compile` before it, REPLACES the output file, header and all.
    ...
    So `make lock` goes through `scripts/lock.py`, which re-applies the header after writing the
    file, and this asserts the header actually survived rather than trusting that it will.

    What is asserted is the header's load-bearing content, not its opening adjective. Pinning the
    exact first line made this fail on the wording change alone when the repo moved from
    compiling each lockfile separately to exporting both from one `uv.lock`, which is a rename
    reported as a missing header: the loudest possible signal for the one kind of edit that is
    definitely safe, and silence is what this test is for.
    """
    text = (REPO_ROOT / name).read_text(encoding="utf-8")
    assert text.startswith("# ") and "DO NOT hand-edit: run `make lock`" in text.split("\n")[0], (
        f"{name} has lost its header. uv overwrites the whole file, so run `make lock` "
