...
from aml_alert_triage.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    build_agent_card,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "aml-alert-triage"
...
def test_the_agent_path_routes_the_outcome_rather_than_only_flagging_it() -> None:
    result = triage_alert(
        "FCC-1001",
    ...
    assert result["review_ref"], "the agent surface flagged an outcome it never routed"


def test_the_agent_path_routes_even_a_low_band_close() -> None:
    result = triage_alert("FCC-1002", settings=local_settings())
    assert result["requires_human_review"] is True
    assert result["review_ref"], "every AML outcome, including a close, reaches a human"


def test_the_alert_queue_tool_lists_open_alerts() -> None:
    result = list_alert_queue(tenant=FIXTURE_TENANT, settings=local_settings())
    ids = [row["alert_id"] for row in result["alerts"]]
    assert "FCC-1001" in ids


def test_the_alert_queue_tool_lists_nothing_for_a_foreign_tenant() -> None:
    """The tool is the widest read surface in the repo: the MODEL chooses the argument.

    So the scope is not the tool's to widen. A tenant that owns nothing lists nothing, and with
    no tenant asserted and none configured it lists nothing rather than everything, which is the
    fail-closed direction.
    """
    assert list_alert_queue(tenant="other-bank", settings=local_settings())["alerts"] == []
    unconfigured = local_settings(tenant="")
    assert list_alert_queue(settings=unconfigured)["alerts"] == []
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    FCC-1001's alert narrative carries a synthetic NRIC; the tool result masks it on the way to
    a model, even where the API would return more to the authenticated caller.
    """
    result = triage_alert("FCC-1001", settings=local_settings())
