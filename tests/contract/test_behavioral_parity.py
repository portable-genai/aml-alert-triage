...
from aml_alert_triage.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    result = service.triage(sample_cases.PII_ALERT, actor=sample_cases.ACTOR)
    payload = repr(
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = local.triage_service().triage(sample_cases.ESCALATING_ALERT, actor=sample_cases.ACTOR)
    ...
    with pytest.raises(NotImplementedError):
        onprem.triage_service().triage(sample_cases.ESCALATING_ALERT, actor=sample_cases.ACTOR)
