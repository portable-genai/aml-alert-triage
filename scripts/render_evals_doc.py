...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden alerts.", ""]
    return lines


def _exercised_block() -> list[str]:
    cases = load_jsonl(GOLDEN)
    close = [row for row in cases if row.get("should_close")]
    escalate = [row for row in cases if not row.get("should_close")]
    planted = sum(1 for row in cases if row.get("planted"))
    lines = [
        BLOCKS[1],
        "",
        f"- **{len(cases)} golden alerts** in `eval/datasets/golden_cases.jsonl`, with the band,",
        ...
        f"- **{len(escalate)} that escalate**, one per typology, which is what `typology_recall`",
        "  is measured over.",
        f"- **{len(close)} that a hand-review says should CLOSE**, which is what",
    ...
    for row in close:
        lines.append(f"  - `{row['alert_id']}` {row.get('benign_pattern', '')}")
    lines += [
        "",
        f"- **{planted} alert plants a raw identifier**, so the leak metric has a target it could",
        "  miss.",
        "",
    ]
    return lines
