...
``AMLTRIAGE`` environment prefix behind every ``AMLTRIAGE_PROFILE``-style variable, the
distribution and resource id ``aml-alert-triage`` and the Terraform
...
``aml_alert_triage``, so ``--package`` renames the CLI as well and a
...
_OLD_PACKAGE = "aml_alert_triage"
_OLD_ENV_PREFIX = "AMLTRIAGE"
_OLD_RESOURCE = "aml-alert-triage"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        if old == _OLD_ENV_PREFIX:
            # Only where it IS the environment prefix: ``AMLTRIAGE_NAME``, the
            # ``AMLTRIAGE_*`` glob a comment writes, and the bare token in
...
def main() -> int:
    ...
    ap.add_argument("--env-prefix", required=True, help="new env-var prefix, e.g. ACMEAML")
    ap.add_argument(
        "--resource",
        required=True,
        help="new distribution / resource id, kebab-case, e.g. acme-aml-triage",
    )
    ap.add_argument(
        "--name-prefix",
        default="",
        help="new Terraform name_prefix default, e.g. acme-aml (default: leave it alone)",
    )
    ...
    if not re.fullmatch(r"[A-Z][A-Z0-9_]*", args.env_prefix):
        ap.error("--env-prefix must be an UPPER_SNAKE token, e.g. ACMEAML")
    if not re.fullmatch(r"[a-z0-9]+(-[a-z0-9]+)*", args.resource):
        ap.error("--resource must be a kebab-case id, e.g. acme-aml-triage")
    if args.name_prefix and not re.fullmatch(r"[a-z][a-z0-9-]{2,18}", args.name_prefix):
        ...
        ap.error("--name-prefix must match ^[a-z][a-z0-9-]{2,18}$, e.g. acme-aml")
