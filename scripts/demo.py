...
from aml_alert_triage.domain import (
    kernel,
    models,
)
from aml_alert_triage.domain.pii import (
    JURISDICTIONS,
)
...
SERVICE_NAME = "AML Alert Triage"
CATALOG_ID = "aml-alert-triage"
REPOSITORY = "aml-alert-triage"
...
#: A planted identifier, present in the FCC-1001 alert narrative, so the redaction panel has an
#: independent literal to look for rather than trusting the pattern pack to agree with itself.
PLANTED_NRIC = "S1234567D"

#: The seeded fictional alerts the demo triages, by id (see adapters/local/_fixtures.py):
#: FCC-1002 bands LOW (close), FCC-1003 bands CRITICAL (escalate), FCC-1001 carries the NRIC.
ROUTINE_ALERT_ID = "FCC-1002"
ESCALATING_ALERT_ID = "FCC-1003"
PII_ALERT_ID = "FCC-1001"
...
STEPS: tuple[Step, ...] = (
    ...
    Step(
        key="routine",
        label="A low-risk alert: banded CLOSE, cited, still human-approved",
        narration=(
            "A low-band alert whose engine recommendation is CLOSE. The score, the band and the "
            "recommendation come from deterministic code, never from a model. It is STILL routed "
            "for human sign-off: an AML close is a consequential disposition, not an auto-close."
        ),
    ),
    Step(
        key="escalation",
        label="A high-risk alert: escalated to SAR AND routed (rule R8)",
        narration=(
            "A critical-band alert: two typologies fired and the engine recommends a SAR. "
            "Setting the flag is not the escalation; routing is. The result is handed to the "
            "human-review console in the same call that produced it, with a reference recording "
            "where it went."
        ),
    ),
    Step(
...
class DemoRun:
    ...
    def __init__(self, workdir: Path | None = None) -> None:
        ...
        self.service = self.container.triage_service()
    ...
    def _step_routine(self) -> Produced:
        return self._triage_panels(ROUTINE_ALERT_ID)

    def _step_escalation(self) -> Produced:
        return self._triage_panels(ESCALATING_ALERT_ID)

    def _step_redaction(self) -> Produced:
        panels, facts = self._triage_panels(PII_ALERT_ID)
    ...
    def _triage_panels(self, alert_id: str) -> Produced:
        alert = self.container.alert_feed.fetch_alert(alert_id, tenant=FIXTURE_TENANT)
        result = self.service.triage(alert, actor=ACTOR)
        self.cases += 1
        # Every AML outcome is consequential and routes to a human (rule R8), in the same call.
        self.escalated += 1
        review_ref = self.container.review_router.route(result, maker=ACTOR, tenant=TENANT)
        self.routed += 1
        consistent = bool(review_ref) and result.requires_human_review
        uplift_rows = tuple(
            Row("+ " + hit.typology_id, str(hit.uplift) + "  " + hit.measure)
            for hit in result.typology_hits
        )
        decision = Panel(
            title="Decision: " + result.subject,
            rows=(
                Row("Alert", result.alert_id),
                Row("Base score", str(result.base_score)),
                *uplift_rows,
                Row("Score", str(result.score)),
                Row("Band", result.band.value, "bad" if result.band.value != "low" else "ok"),
                Row("Recommendation", result.recommendation.value),
                Row("Requires human review", str(result.requires_human_review)),
                Row("Routed to review", review_ref, "ok" if consistent else "bad"),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "The score, band and recommendation are computed by pure stdlib code and are "
                "replayable. A model narrates this result; it never produces a number."
            ),
...
def _exit_review(container: Any) -> Any:
    citation = kernel.Citation(
        source_id="fatf_r20_str",
        title="FATF Recommendation 20",
        snippet="reporting of suspicious transactions",
    )
    return container.review_router.route(
        models.TriageAssessment(
            ...
            base_score=0.20,
            score=0.65,
            band=kernel.Severity.HIGH,
            recommendation=models.Recommendation.ESCALATE_SAR,
            narrative="Structuring pattern; recommend SAR.",
            as_of=kernel.utcnow().date(),
...
def _exit_alert_feed(container: Any) -> Any:
    return container.alert_feed.fetch_alert("FCC-1001", tenant=FIXTURE_TENANT)


def _exit_warehouse(container: Any) -> Any:
    return container.warehouse.fetch_window("Acme Holdings (FICTIONAL)", kernel.utcnow().date())


def _exit_retrieval(container: Any) -> Any:
    return container.retrieval.retrieve(
        models.RetrievalQuery(text="structuring", typology_ids=("structuring",))
    )


def _exit_narration(container: Any) -> Any:
    return container.narration.narrate(
        models.NarrationRequest(
            subject="Acme Holdings (FICTIONAL)",
            band=kernel.Severity.HIGH,
            score=0.65,
            recommendation=models.Recommendation.ESCALATE_SAR,
