...
def _check_routine(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if facts.get("band") != "low":
        problems.append("the low-risk alert did not band low: " + str(facts.get("band")))
    if facts.get("recommendation") != "close":
        problems.append("the low-risk alert was not a proposed close")
    # In this vertical EVERY outcome is consequential: a proposed close still reaches a human.
    if not facts.get("requires_human_review"):
        problems.append("a proposed close skipped human review")
    if not facts.get("review_ref"):
        problems.append("rule R8: even a proposed close must be routed for sign-off")
    return problems
