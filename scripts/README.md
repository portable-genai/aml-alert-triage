...
| Script | What it does |
|---|---|
| `render_evals_doc.py` | Regenerates the derived half of `docs/evals.md` from the rubrics and the golden set (`make evals-doc`). `--check` runs inside `make gate`, so a bar that moved without the page being re-rendered fails the build instead of leaving a reader a page that quietly stopped being true. |
...
| `rename_fork.py` | The mechanical half of adopting this repo as a base: rewrites the package name (which is also the console-script name), the `AMLTRIAGE_` environment prefix, the distribution and resource id, and the Terraform `name_prefix` default, in one pass. It prints a plan and writes nothing without `--yes`. The decisions it cannot make for you are the checklist in `docs/ADOPTING.md`. |
