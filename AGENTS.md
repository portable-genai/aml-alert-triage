...
After any dependency change, run `make lock` and commit `uv.lock` together with both
exported lockfiles. `uv.lock` is the resolution; the two `requirements-*.lock` files are
exports of it for the Dockerfile, CI and every `pip install -r`. An uncommitted resolution
...
- `src/aml_alert_triage/domain/` is PURE stdlib. `kernel.py` holds the vertical-neutral
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `AMLTRIAGE_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `AMLTRIAGE_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `AMLTRIAGE_PROFILE` (mentioning it in a refusal message is wanted, reading
