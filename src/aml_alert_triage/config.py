...
One env var (``AMLTRIAGE_PROFILE``) selects the adapter family for every
...
from .domain.policy import TriagePolicy
from .domain.triage_service import TriageService
from .domain.typology_engine import TypologyEngine
from .envread import boolean_setting, setting_or_default
from .ports.alert_feed import AlertFeedPort
...
from .ports.warehouse import WarehousePort
from .typology_packs import load_packs

_PROFILE_ENV = "AMLTRIAGE_PROFILE"
_SETTINGS_ENV = "AMLTRIAGE_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``AMLTRIAGE_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("AMLTRIAGE_PROFILE", "local")``: that fallback reads an
...
_PKG = "aml_alert_triage"

DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    "alert_feed": {
        "local": f"{_PKG}.adapters.local.alert_feed:LocalAlertFeed",
        "gcp": f"{_PKG}.adapters.gcp.alert_feed:CloudAlertFeed",
        "onprem": f"{_PKG}.adapters.onprem.alert_feed:OnPremAlertFeed",
    },
    "warehouse": {
        "local": f"{_PKG}.adapters.local.warehouse:LocalWarehouse",
        "gcp": f"{_PKG}.adapters.gcp.warehouse:CloudWarehouse",
        "onprem": f"{_PKG}.adapters.onprem.warehouse:OnPremWarehouse",
    },
    "retrieval": {
        "local": f"{_PKG}.adapters.local.retrieval:LocalRetrieval",
        "gcp": f"{_PKG}.adapters.gcp.retrieval:CloudRetrieval",
        "onprem": f"{_PKG}.adapters.onprem.retrieval:OnPremRetrieval",
    },
    "narration": {
        "local": f"{_PKG}.adapters.local.narration:LocalNarration",
        "gcp": f"{_PKG}.adapters.gcp.narration:CloudNarration",
        "onprem": f"{_PKG}.adapters.onprem.narration:OnPremNarration",
    },
    "audit": {
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``AMLTRIAGE_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "AMLTRIAGE_REVIEW_ROUTING"
...
class Settings:
    ...
    project_id: str = ""
    #: Path to the typology rule pack. Empty means the shipped reference pack
    #: (``rulepacks/typologies.yaml``); an adopter points this at its own pack to add markets or
    #: tighten thresholds without a code edit.
    typology_pack_path: str = ""
    #: Bank-owned triage policy (the score baseline, band thresholds, recommendation map). Empty
    #: means the reference defaults in ``domain/policy.py``; a ``policy:`` block in the settings
    #: file overrides any subset of it (practices check B4).
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            typology_pack_path=str(data.get("typology_pack_path") or ""),
...
class Container:
    ...
    @cached_property
    def alert_feed(self) -> AlertFeedPort:
        adapter = self._bind("alert_feed")
        assert isinstance(adapter, AlertFeedPort)
        return adapter

    @cached_property
    def warehouse(self) -> WarehousePort:
        adapter = self._bind("warehouse")
        assert isinstance(adapter, WarehousePort)
        return adapter
    ...
    def build_engine(self) -> TypologyEngine:
        """The pure deterministic typology engine, built from the pack file and bank policy.

        A plain method, not a ``cached_property``: the port-parity guard treats every
        ``cached_property`` on the Container as a PORT accessor, and the engine is domain
        machinery, not a port. Building it loads the pack YAML (edge work); the engine it
        constructs is pure and takes only frozen dataclasses.
        """
        pack_path = (
            Path(self.settings.typology_pack_path) if self.settings.typology_pack_path else None
        )
        packs = load_packs(pack_path)
        policy = TriagePolicy.from_mapping(self.settings.policy)
        return TypologyEngine(packs=packs, policy=policy)

    def triage_service(self) -> TriageService:
        """Assemble the triage orchestrator from the bound ports and the engine."""
