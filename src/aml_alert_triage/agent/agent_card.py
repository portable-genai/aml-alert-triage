...
SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        ...
        name="Alert triage",
        description=(
            "Score one monitoring alert's transaction window into a deterministic risk band and "
            "recommendation, draft a grounded SAR narrative, record an already-redacted audit "
            "event, and ROUTE the result to human review (rule R8). The band and the "
            "recommendation are computed by pure stdlib code, never by a model."
        ),
    ),
    AgentSkill(
        id="draft_sar_narrative",
        name="SAR narrative drafting",
        description=(
            "Draft the cited suspicious-activity-report narrative for one alert, restating only "
            "figures the deterministic engine computed. An invented number is discarded with the "
            "whole draft; every claim carries a citation."
        ),
    ),
    AgentSkill(
        id="list_alert_queue",
        name="Alert queue",
        description=(
            "List the open monitoring-alert queue: alert ids and subjects, with no scoring. The "
            "queue is read from the alert feed and carries no transaction detail."
        ),
    ),
    AgentSkill(
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://aml-alert-triage.{settings.region}.internal.example/a2a"
