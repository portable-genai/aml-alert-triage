...
``AMLTRIAGE_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``AMLTRIAGE_S2S_TOKEN`` is wrong in the most dangerous direction:
...
_CORS_ORIGINS_ENV = "AMLTRIAGE_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "AMLTRIAGE_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "AMLTRIAGE_S2S_AUDIENCE"
_API_HOST_ENV = "AMLTRIAGE_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``AMLTRIAGE_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``AMLTRIAGE_PROFILE`` must never be allowed to
    ...
    ``AMLTRIAGE_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "AMLTRIAGE_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "aml-alert-triage"
...
#
# Note what is NOT in this expression: AMLTRIAGE_S2S_TOKEN. A service
...
def triage(
    ...
    """Triage one alert; the audit actor is the verified principal, never the request body.
    ...
    try:
        alert = container.alert_feed.fetch_alert(request.alert_id, tenant=principal.tenant)
    except KeyError as exc:
        # 404, not 403: an alert owned by another tenant must be indistinguishable from one that
...
def list_alerts(
    ...
    """List the principal's own open-alert queue (``list_alert_queue``): ids and subjects only.

    Scoped to ``principal.tenant``. An unscoped queue is the read-by-id defect with no id to
    guess: it hands a caller from another bank the whole inventory, subjects included.
    """
    container = _container()
    return [
        AlertSummary.from_domain(a)
        for a in container.alert_feed.list_open_alerts(tenant=principal.tenant)
    ]
