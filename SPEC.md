...
- **Determinism**: the risk score, band and the close / request-info / escalate-to-SAR
  recommendation are produced by the pure `TypologyEngine` (`domain/typology_engine.py`) over
  typology packs as data (`rulepacks/typologies.yaml`) and bank-owned policy (`domain/policy.py`).
  Given the same alert, the whole `ScoreCard` is byte-identical, so an investigator can recompute
  it. The LLM narrates the finished assessment and produces no number; a drafted figure the engine
  never computed is discarded with the whole draft (the groundedness rule). The engine code carries
...
Every metric scores against the dataset's OWN `expected_*` oracle (written from the fixtures,
never from pipeline output) and is proven able to go red in `tests/unit/test_not_falsely_green.py`.
- `recommendation_accuracy >= 0.80` (engine recommendation vs the golden oracle)
- `typology_recall >= 0.90` (every expected typology fired)
- `groundedness >= 1.0` (the narrative states no figure the engine did not compute)
- `review_safety >= 1.0` (every outcome requires human review, rule R8)
